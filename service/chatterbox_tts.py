...
def _load_pretrained_sync():
    return ChatterboxTurboTTS.from_pretrained(device=_device)
...
async def load_global_model():
    ...
    if _global_model is None:
        ...
        _global_model = await asyncio.to_thread(_load_pretrained_sync)
...
class ChatterboxTTS:
    ...
    async def save_to_file(self, wav, sr, output_path):
        """Save audio to file"""
        await asyncio.to_thread(ta.save, output_path, wav.cpu(), sr)
